...
function preferred_units(::Type{D}) where D <:AbstractDimensions 
    error("Function `preferred_units` not defined for type $(D): \n
    This function is usually defined in a unit registry. Perhaps there is no unit registry for $(D) or its unit registry was not properly configured")
end
